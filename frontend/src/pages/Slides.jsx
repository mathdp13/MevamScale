...
import { Plus, Trash2, ImageOff, MonitorPlay, Calendar, ToggleLeft, ToggleRight, Upload, CalendarClock } from 'lucide-react';
...
function FormImagem({ preview, onFile, onRemover, children }) {
  return (
    <div>
      {preview ? (
        <div>
...
function Slides() {
  ...
  const handleFile = (setter, setterPreview) => (e) => {
  ...
  const criarSlide = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.error || 'Erro ao criar slide.');
    } finally {
                      ...
                      return (
                        <div key={s.id} className="flex items-center gap-3 bg-white/5 rounded-xl p-3">
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            ...
                            <button onClick={() => deletarSlide(s.id)} className="text-gray-600 hover:text-red-400 transition-colors">
                              <Trash2 size={14} />
                            </button>
                          </div>
                        </div>
              ...
              <div className="bg-[#0a1a33] rounded-2xl border border-white/5 p-6">
                ...
                <div className="space-y-3">
                  ...
                  <FormImagem
                    ...
                    onFile={handleFile(setSlideFile, setSlidePreview)}
                    onRemover={() => { setSlideFile(null); setSlidePreview(''); }}
                  >
                  ...
                  <button onClick={criarSlide} disabled={salvandoSlide} className="w-full bg-blue-600 hover:bg-blue-500 text-white py-3 rounded-2xl font-bold text-sm transition-all active:scale-95 disabled:opacity-50">
                    <Plus size={14} className="inline mr-1" />
                    {salvandoSlide ? 'Salvando...' : 'Adicionar slide'}
                  </button>
                </div>
              </div>
            </>
            ...
            <>
              ...
              <div className="bg-[#0a1a33] rounded-2xl border border-white/5 p-6">
                ...
                <div className="space-y-3">
                  ...
                  <FormImagem
                    preview={evPreview}
                    onFile={handleFile(setEvFile, setEvPreview)}
                    onRemover={() => { setEvFile(null); setEvPreview(''); }}
                  >
